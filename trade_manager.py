class TradeManager():
    ...
    def close_trades(self, pairs_to_close):
        ...
        ids_to_close = [
            x.trade_id for x in open_trades if x.instrument in pairs_to_close
        ]

        self.log_message(
            f"TradeManager:close_trades() pairs_to_close:{pairs_to_close} ")
        self.log_message(
            f"TradeManager:close_trades() open_trades:{open_trades} ")
        self.log_message(
            f"TradeManager:close_trades() ids_to_close:{ids_to_close} ")

        for t in ids_to_close:
            ok = self.api.close_trade(t)
            if ok == False:
                self.log_message(
                    f"TradeManager:close_trades() {t} FAILED TO CLOSE ")
            else:
